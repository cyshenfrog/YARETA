...
namespace HinputClasses {
	...
	public class Stick {
		...
		/// <summary>
		/// Disable a stick so that Hinput stops tracking it.
		/// </summary>
		public void Disable() {
			Reset();
			isEnabled = false;
		...
		public static implicit operator Vector2 (Stick stick) => stick.position;
		public static implicit operator bool (Stick stick) => stick.inPressedZone;
		...
		public Stick(string name, Gamepad gamepad, int index) {
			this.name = gamepad.name + "_" + name;;
		...
		protected virtual Vector2 GetPosition() {
			...
			if (Settings.stickDeadZone > Settings.stickMaxZone) return Vector2.zero;
			
			float magnitude = positionRaw.magnitude;
			if (Settings.stickDeadZone > magnitude) return Vector2.zero;

			return (magnitude.Prel(Settings.stickDeadZone, Settings.stickMaxZone)*positionRaw.normalized).Clamp(-1, 1);
		}
		...
		public void Update () {
			...
			positionRaw = gamepad.player.sticks[index];
		...
		/// <summary>
		/// The position of a stick along the horizontal axis (all the way left=-1, all the way right=1).
		/// </summary>
		public float horizontal => position.x;

		/// <summary>
		/// The position of a stick along the vertical axis (all the way down=-1, all the way up=1).
		/// </summary>
		...
		public Vector3 worldPositionCamera { 
			get { 
				...
				catch { HinputClasses.Internal.ErrorMessages.NoCameraFoundError(); }
				return Vector2.zero;
			} 
		}
		...
		/// </summary>
		public Vector3 worldPositionFlat => new Vector3 (horizontal, 0, vertical);
